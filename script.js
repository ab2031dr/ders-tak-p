...
let CURRENT_USER = null;   // firebase.User veya null
...
/* Firebase auth state (firebase-init.js tetikler) */
window.onCloudAuth = async (user)=>{
  CURRENT_USER = user || null;
  CURRENT_UID  = user ? user.uid : null;
  refreshAuthUI();
  if(CURRENT_UID){
    // buluttan ayar & istatistik çek ve local'e yaz
    try{
...
function setStudy(o){
  localStorage.setItem(keyFor("studyData"),JSON.stringify(o));
  if(CURRENT_UID) cloud.db.setStudyData(CURRENT_UID,o).catch(()=>{});
}
function addStudyMinutesTo(dateIso,minutes){
  if(minutes<=0)return;
  const d=getStudy(); d[dateIso]=(d[dateIso]||0)+minutes; setStudy(d);
}
...
function setQuestions(o){
  localStorage.setItem(keyFor("questionData"),JSON.stringify(o));
  if(CURRENT_UID) cloud.db.setQuestionData(CURRENT_UID,o).catch(()=>{});
}
...
function saveSettings(){
  const data = {
    studySec, breakSec, longBreakSec, totalRounds, totalSets, soundOn, alarmOn
  };
...
let weekChart = null;
function refreshTotals(){const data=getStudy(),today=todayKey();document.getElementById("todayTotal").textContent=Math.round(data[today]||0);const weekDates=last7Dates();const weekSum=weekDates.reduce((s,d)=>s+(data[d]||0),0);document.getElementById("weekTotal").textContent=Math.round(weekSum);}
...
function updateRoundSetLabels(){document.getElementById("roundLabel").textContent=`${completedRounds} / ${totalRounds}`;document.getElementById("setLabel").textContent=`${completedSets} / ${totalSets}`;}
...
(function init(){
  bindUI();
  startLiveClock();
  // auth state geldiğinde onCloudAuth içinden UI hazırlanıyor
})();
